...
[← Unit 0 — First Tiny-LM Run](/unit-00) · [Diagnostic & learning path →](/diagnostic) · [Glossary](/glossary)
...
| `--prompt must contain at least one character` | a language model needs at least one context token | provide a non-empty `--prompt` |

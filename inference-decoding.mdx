...
their sum. At temperature <InlineMath tex={"T = 1"} /> the exponentials are 7.3891, 2.7183, 1.6487,
1.0000 and 0.3679, the sum is 13.1239, and the probabilities follow. At <InlineMath tex={"T = 0.5"} />
...
beats the second by <InlineMath tex={"e^{1} = 2.718"} /> at <InlineMath tex={"T = 1"} /> and by <InlineMath tex={"e^{2} = 7.389"} /> at <InlineMath tex={"T = 0.5"} />.
As <InlineMath tex={"T"} /> approaches zero the first entry approaches 1, so
...
matching one-hot vector. For <InlineMath tex={"T > 0"} />, `distribution` subtracts the largest logit
...
Sampling at <InlineMath tex={"T = 1"} /> gives the fifth token 2.8 % of the draws. Over a hundred tokens
that tail adds up, and one improbable token drags every later token with it. Two
masks cut the tail. Top-k keeps the <InlineMath tex={"k"} /> largest entries and renormalises: with
<InlineMath tex={"k = 2"} /> the first two entries hold 0.5630 + 0.2071 = 0.7701 of the mass, so they
become 0.5630 / 0.7701 = 0.7311 and 0.2689. Top-p (nucleus) sorts the
probabilities and keeps the shortest prefix whose cumulative mass reaches <InlineMath tex={"p"} />: the
running sums are 0.5630, 0.7701, 0.8958, 0.9720, so <InlineMath tex={"p = 0.9"} /> keeps four entries
...
gap to `'l'` and `'o'`. The three continuations at <InlineMath tex={"T = 0.7"} /> with seeds 1, 2 and 3
are `follows`, `reveals` and `predict`, one per leader. At <InlineMath tex={"T = 1"} /> the full softmax
...
<InlineMath tex={"t"} /> tokens, of which it keeps the last row and discards the rest. Count the work at
prefix length <InlineMath tex={"t"} /> with width <InlineMath tex={"D = 32"} />. Each block projects <InlineMath tex={"t"} /> positions to queries,
keys and values, <InlineMath tex={"3tD^2"} /> multiply-adds, forms a <InlineMath tex={"t \\times t"} /> score matrix per head,
and runs the FFN over <InlineMath tex={"t"} /> positions, <InlineMath tex={"8tD^2"} /> more. Step <InlineMath tex={"t + 1"} /> does all of it
again for the first <InlineMath tex={"t"} /> positions, although the causal mask guarantees that their
hidden states, keys and values have not changed: position <InlineMath tex={"i"} /> never reads a
position after <InlineMath tex={"i"} />. Per step the projections cost <InlineMath tex={"O(t)"} /> and the scores <InlineMath tex={"O(t^2)"} />;
summed over <InlineMath tex={"T"} /> generated tokens that is <InlineMath tex={"O(T^2)"} /> and <InlineMath tex={"O(T^3)"} />. With a cache the
projections cost <InlineMath tex={"O(1)"} /> per step, <InlineMath tex={"O(T)"} /> in total, and only the scores keep
<InlineMath tex={"O(t)"} /> per step, <InlineMath tex={"O(T^2)"} /> in total: <InlineMath tex={"O(T^2)"} /> against <InlineMath tex={"O(T)"} /> for the term that
dominates at this width. At prefix 96 one
block's three projections cost <InlineMath tex={"3 \\cdot 96 \\cdot 1024 = 294{,}912"} /> multiply-adds
...
query against <InlineMath tex={"t"} /> keys, no mask, because only the past is stored. The output
...
asserts a difference below <InlineMath tex={"10^{-4}"} /> at each of 20 steps. It is not exactly zero because a batched <InlineMath tex={"t \\times t"} /> matmul and a
<InlineMath tex={"1 \\times t"} /> one sum in different orders in float32. The timing table measures a
...
the top-5 at <InlineMath tex={"T = 1"} /> for the lab prompt with penalty 1.0 and 1.5, then the greedy
